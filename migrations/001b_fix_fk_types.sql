...
DECLARE
    ...
    v_person_type TEXT;
BEGIN
    SELECT data_type INTO v_expense_type
      FROM information_schema.columns
     WHERE table_name = 'expenses' AND column_name = 'id';
    SELECT data_type INTO v_person_type
      FROM information_schema.columns
     WHERE table_name = 'people' AND column_name = 'id';

    v_expense_type := COALESCE(v_expense_type, 'text');
    v_person_type := COALESCE(v_person_type, 'bigint');

    EXECUTE format('
        CREATE TABLE IF NOT EXISTS shared_expenses (
            id BIGINT PRIMARY KEY,
            expense_id %s REFERENCES expenses(id) ON DELETE CASCADE,
    ...
    EXECUTE format('
        CREATE TABLE IF NOT EXISTS shared_expense_participants (
            ...
            person_id %s REFERENCES people(id) ON DELETE CASCADE,
...
FOR SELECT USING (
    ...
    OR id IN (SELECT "groupId" FROM group_members WHERE user_id = auth.uid()::text)
);
...
FOR SELECT USING (
    ...
    OR "groupId" IN (SELECT id FROM groups WHERE created_by::text = auth.uid()::text)
);
...
FOR UPDATE USING ("groupId" IN (SELECT id FROM groups WHERE created_by::text = auth.uid()::text));

DROP POLICY IF EXISTS "group_members_delete" ON group_members;
CREATE POLICY "group_members_delete" ON group_members
FOR DELETE USING ("groupId" IN (SELECT id FROM groups WHERE created_by::text = auth.uid()::text));
...
FOR SELECT USING (
    created_by::text = auth.uid()::text
    OR group_id IN (SELECT "groupId" FROM group_members WHERE user_id = auth.uid()::text)
    OR id IN (
...
FOR SELECT USING (
    shared_expense_id IN (SELECT id FROM shared_expenses WHERE created_by::text = auth.uid()::text)
...
FOR DELETE USING (group_id IN (SELECT id FROM groups WHERE created_by::text = auth.uid()::text));

-- -----------------------------------------------------------------
-- 12. Ricarica schema cache PostgREST
-- -----------------------------------------------------------------
