...
const DEFAULT_CATEGORIES = [
    { name: 'Alimentari', icon: 'https://cdn-icons-png.flaticon.com/128/3081/3081949.png' },
    { name: 'Igiene e Pulizia', icon: 'https://cdn-icons-png.flaticon.com/128/2553/2553606.png' },
    { name: 'Carburante Auto', icon: 'https://cdn-icons-png.flaticon.com/128/2933/2933909.png' },
    { name: 'Carburante Moto', icon: 'https://cdn-icons-png.flaticon.com/128/2933/2933924.png' },
    { name: 'Sanitarie', icon: 'https://cdn-icons-png.flaticon.com/128/2966/2966327.png' },
    { name: 'Bolletta Acqua', icon: 'https://cdn-icons-png.flaticon.com/128/3053/3053896.png' },
    { name: 'Bolletta Luce', icon: 'https://cdn-icons-png.flaticon.com/128/2983/2983973.png' },
    { name: 'Bolletta Gas', icon: 'https://cdn-icons-png.flaticon.com/128/2916/2916262.png' },
    { name: 'Bolletta Rifiuti', icon: 'https://cdn-icons-png.flaticon.com/128/2983/2983955.png' },
    { name: 'Bolletta Condominio', icon: 'https://cdn-icons-png.flaticon.com/128/2966/2966471.png' },
    { name: 'Bolletta Telefonia', icon: 'https://cdn-icons-png.flaticon.com/128/2983/2983803.png' },
    { name: 'Mutuo', icon: 'https://cdn-icons-png.flaticon.com/128/2933/2933905.png' },
    { name: 'Formazione', icon: 'https://cdn-icons-png.flaticon.com/128/2983/2983981.png' },
    { name: 'Varie', icon: 'https://cdn-icons-png.flaticon.com/128/2983/2983805.png' }
];

const ICON_OPTIONS = ['https://cdn-icons-png.flaticon.com/128/3081/3081949.png', 'https://cdn-icons-png.flaticon.com/128/2933/2933909.png', 'https://cdn-icons-png.flaticon.com/128/2966/2966327.png', 'https://cdn-icons-png.flaticon.com/128/2983/2983981.png', 'https://cdn-icons-png.flaticon.com/128/2983/2983805.png'];
...
function renderIconOptions() {
    ...
    if (!select) return;
    select.innerHTML = '';
    ICON_OPTIONS.forEach((iconUrl, index) => {
        const option = document.createElement('option');
        option.value = iconUrl;
        option.innerText = 'Icona ' + (index + 1);
...
async function initCategories() {
    ...
    userCategories = dbCats.map(c => c.name).sort();
    categoryIconMap = {};
    dbCats.forEach(c => { categoryIconMap[c.name] = c.icon || ICON_OPTIONS[0]; });
...
function getCatIcon(catName) {
    const iconUrl = categoryIconMap[catName] || ICON_OPTIONS[0];
    if (iconUrl.startsWith('http')) {
        return `<img src="${iconUrl}" style="width:20px;height:20px;vertical-align:middle;">`;
    }
    return iconUrl;
}

function renderCategoriesAdmin() {
    const list = $('categoriesAdminList');
    if (!list) return;
    list.innerHTML = '';
    const dbCats = Object.keys(categoryIconMap).sort();
    dbCats.forEach(cat => {
        const row = document.createElement('div');
        row.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:8px;border-bottom:1px solid #e2e8f0;';
        row.innerHTML = `
            <div>${getCatIcon(cat)} <span style="margin-left:8px;font-weight:bold;">${escapeHtml(cat)}</span></div>
            <button class="btn-del" style="padding:4px 8px;font-size:12px;">✕</button>`;
        row.querySelector('.btn-del').addEventListener('click', () => deleteCategory(cat));
        list.appendChild(row);
    });
}

function populateCategorySelects() {
    const select = $('expCategory');
    if (select) {
        select.innerHTML = '';
...
async function saveCategory() {
    ...
    const catIcon = $('newCatIcon')?.value;
    if (!catName) {
        alert('Inserisci un nome per la categoria.');
        return;
    }
    await db.categories.put({ name: catName, icon: catIcon });
    $('newCatName').value = '';
    await initCategories();
    await safeCloudCall('save category', syncLocalToSupabaseFirstTime);
}

async function deleteCategory(catName) {
    if (!confirm(`Sei sicuro di voler eliminare la categoria "${catName}"?`)) return;
    await db.categories.delete(catName);
    await safeCloudCall('delete category cloud', () => supabase.from('categories').delete().eq('name', catName));
    await initCategories();
}
...
async function callAIEndpoint(promptText, responseBoxId, btnId) {
    ...
    box.innerText = '🤖 Elaborazione in corso via OpenRouter...';
    ...
    try {
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            headers: {
                ...
                'Authorization': `Bearer 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`
            },
            body: JSON.stringify({
        ...
        box.innerText = json.choices?.[0]?.message?.content || '❌ Risposta IA non valida.';
    } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ALTRI EVENT LISTENERS MANCANTI
    const btnSaveCategory = document.getElementById('btnSaveCategory');
    if (btnSaveCategory) btnSaveCategory.addEventListener('click', saveCategory);

    const btnAddExpense = document.getElementById('btnAddExpense');
    if (btnAddExpense && typeof addExpense === 'function') btnAddExpense.addEventListener('click', addExpense);

    const btnAddIncome = document.getElementById('btnAddIncome');
    if (btnAddIncome && typeof addIncome === 'function') btnAddIncome.addEventListener('click', addIncome);

    const btnAnalyseIA = document.getElementById('btnAnalyseIA');
    if (btnAnalyseIA && typeof runFinancialAnalysisIA === 'function') btnAnalyseIA.addEventListener('click', runFinancialAnalysisIA);

    const btnHistoryIA = document.getElementById('btnHistoryIA');
    if (btnHistoryIA && typeof runHistoryAnalysisIA === 'function') btnHistoryIA.addEventListener('click', runHistoryAnalysisIA);

    const btnFutureIA = document.getElementById('btnFutureIA');
    if (btnFutureIA && typeof runFuturePredictionIA === 'function') btnFutureIA.addEventListener('click', runFuturePredictionIA);

    const btnSimulateFuture = document.getElementById('btnSimulateFuture');
    if (btnSimulateFuture && typeof renderFutureProjections === 'function') btnSimulateFuture.addEventListener('click', () => renderFutureProjections(true));

    const btnResetFuture = document.getElementById('btnResetFuture');
    if (btnResetFuture && typeof resetFutureSimulation === 'function') btnResetFuture.addEventListener('click', resetFutureSimulation);

    const btnAddAnnualDeadline = document.getElementById('btnAddAnnualDeadline');
    if (btnAddAnnualDeadline && typeof addAnnualDeadline === 'function') btnAddAnnualDeadline.addEventListener('click', addAnnualDeadline);

    const btnExportJSON = document.getElementById('btnExportJSON');
    if (btnExportJSON && typeof exportBackupJSON === 'function') btnExportJSON.addEventListener('click', exportBackupJSON);

    const importFileInput = document.getElementById('importFileInput');
    if (importFileInput && typeof importBackupJSON === 'function') importFileInput.addEventListener('change', importBackupJSON);

    const btnInstallApp = document.getElementById('btnInstallApp');
    if (btnInstallApp && typeof installPWA === 'function') btnInstallApp.addEventListener('click', installPWA);

    const pushNotifToggle = document.getElementById('pushNotifToggle');
    if (pushNotifToggle && typeof togglePushNotifications === 'function') pushNotifToggle.addEventListener('change', togglePushNotifications);

    const searchInput = document.getElementById('searchInput');
    if (searchInput && typeof handleSearch === 'function') searchInput.addEventListener('input', handleSearch);

    const btnClearAllFilters = document.getElementById('btnClearAllFilters');
    if (btnClearAllFilters && typeof clearAllFilters === 'function') btnClearAllFilters.addEventListener('click', clearAllFilters);

    const startCycleDayInput = document.getElementById('startCycleDay');
    if (startCycleDayInput) startCycleDayInput.addEventListener('change', (e) => {
        localStorage.setItem('global_start_cycle_day', e.target.value);
        location.reload();
    });

    const btnDepositSavings = document.getElementById('btnDepositSavings');
    if (btnDepositSavings && typeof depositToSavingsGoal === 'function') btnDepositSavings.addEventListener('click', depositToSavingsGoal);

    const btnAddSavingsGoal = document.getElementById('btnAddSavingsGoal');
    if (btnAddSavingsGoal && typeof addSavingsGoal === 'function') btnAddSavingsGoal.addEventListener('click', addSavingsGoal);

    // INIZIALIZZAZIONE DATI E APP
    initApp();
    setupSupabaseAuth();
});