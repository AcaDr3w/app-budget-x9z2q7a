...
## 📱 ANALISI mobile — STATO ATTUALE v4 (2026-08-14)
- **Layout stretto**: `#history-tab.active` = flex column `justify-content: space-between !important`, `gap:8px`, `padding:12px`, `min-height:0`, `overflow:hidden` (height 100% del `.container` = `calc(100dvh - header 60px - nav 71px)`). MAI aggiungere margini verticali tra i 5 blocchi (header, banner, KPI, carousel, barra) — tutto `flex-shrink:0` tranne carousel `flex:0 1 auto; max-height:220px`.
- **Carousel anomalie**: `max-height: 220px`, flex column: `.anomaly-header` (8px 12px 0) + `.anomaly-window` (flex:1, slides fade 0.6s UNA visibile) + `.anomaly-dots` (button 6px, attivo `#7c3aed` 16px, `goAnomalySlide(i)` stop→jump→restart 3.5s). **Sparkline dentro carousel = 64px** (intervallo 60-80). NIENTE label assoluta.
