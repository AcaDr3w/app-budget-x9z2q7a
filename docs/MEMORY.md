...
## 📱 ANALISI mobile — STATO ATTUALE v3 (2026-08-14)
- **Banner IA**: `.ai-insight-card` = div role=button con `.ai-insight-marquee` (overflow:hidden, flex:1, **SENZA animazione**) > `.ai-insight-track` (**inline-flex + width:max-content + `aiMarquee` 14s translateX(-50%)** — la transform DEVE stare sul track, mai sul marquee, altrimenti -50% = mezza finestra e il loop salta) > 2 span `.ai-insight-text` identici (padding-right:28px). ↻ `.ai-insight-refresh` 36px/13px (stopPropagation → `generateInsightCard(true)`).
- **KPI**: `.kpi-swipe` (flex, overflow-x:auto, `scroll-snap-type:x mandatory`, scrollbar nascosta) con 3 `.kpi-card` flex:0 0 78% snap-start: Media Uscite (`#trendAvg*`), Top Crescita (`#trendTopCat*` — SOLO delta positivo >1%, mai "in calo"; altrimenti `—`/Nessuna crescita), **Scostamento Budget** (`#budgetDiffValue`/`#budgetDiffDelta` = ΣtotalActual−ΣtotalPlanned dei record `db.months` nel periodSet; "+85 €" + "▲ +8% vs Budget" rosso / "▼ −5%" verde / "= In linea" grigio).
- **Carousel anomalie**: `.anomaly-carousel` `flex:1 1 35%` (min-height:0) flex column → `.anomaly-header` (titolo) + `.anomaly-window` (flex:1, slides assolute fade 0.6s, UNA visibile) + `.anomaly-dots` (button 6px, attivo `#7c3aed` 16px; onclick `goAnomalySlide(i)` stop→jump→restart 3.5s). NIENTE label assoluta (sovrapponeva il nome categoria). Timer `anomalyTimer`, `stopAnomalyCarousel()` fuori dal tab.
- **`isFixed(e)`** = `isRecurring || recurringGroupId || /(mutuo|affitto|bollett|tass|luce|gas|acqua|telefon|internet|canone|assicuraz)/i.test(category)` — regex case-insensitive per barra Fisse/Variabili E esclusione anomalie. MAI solo-flag (dava 0% irrealistico).
- **No-scroll**: `#history-tab.active` flex column overflow:hidden; header/banner/KPI/barra flex-shrink:0, carousel basis 35% shrink:1. Desktop ≥768: ramo `renderGlobalHistory` intatto (hide-mobile/hide-desktop).
