...
## 📱 ANALISI mobile — STATO ATTUALE v5 (2026-08-14)
- **Layout stretto**: `#history-tab.active` = flex column `gap:8px`, `padding:12px`, `min-height:0`, `overflow:hidden` (height 100% del `.container` = `calc(100dvh - header 60px - nav 71px)`). MAI aggiungere margini verticali tra i 5 blocchi (header, banner, KPI, carousel, barra) — header/banner/KPI/barra `flex-shrink:0`, **carousel `flex:1 1 auto; min-height:0`** (occupa TUTTO lo spazio rimanente, spec). NIENTE `justify-content: space-between` sul tab.
- **Carousel anomalie = SLIDE ORIZZONTALE**: `.anomaly-window` (flex:1, `overflow:hidden`, `touch-action: pan-y`) > `.anomaly-track` (flex row, `transition: transform .5s cubic-bezier(.4,0,.2,1)`, classe `.dragging` = no transition) > `.anomaly-slide` (`flex:0 0 100%; height:100%`, NIENTE absolute/opacity). `moveAnomalyTo(box, i)` = `translateX(-i*100%)` + dot attivo + aria-hidden. Dots (6px, attivo `#7c3aed` 16px), `goAnomalySlide(i)` = stop→jump→restart 3.5s. **Swipe**: `setupAnomalySwipe(box)` Pointer Events (guard `dataset.swipeBound`, bind post-render), |Δx|>50 → avanti/indietro, altrimenti snap; restart timer. **Sparkline = `clamp(64px, 18vh, 140px)`** (scala con l'area).
- **KPI**: `.kpi-swipe` overflow-x auto snap-x mandatory; `.kpi-card` **`flex:0 0 75%`** snap-start (3 card: Media Uscite `#trendAvg*`, Top Crescita `#trendTopCat*` solo delta>1% altrimenti "Nessuna crescita", Scostamento `#budgetDiff*` = Σactual−Σplanned da `db.months` del periodSet; label **"vs Previsto"**).
- **`isFixed(e)`** = `isRecurring || recurringGroupId || /(mutuo|affitto|bollett|tass|luce|gas|acqua|telefon|internet|canone|assicuraz)/i.test(category)` — barra Fisse/Variabili E esclusione anomalie.
